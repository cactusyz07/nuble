...
export class Application {
  ...
  static async init(rootStore: RootStore) {
    ...
    // create express server to serve game files
    ...
    // Intercept index.html to inject IonicDeeplink mock so Lindo can pass OAuth tokens
    // cleanly to Ankama's WebAuth system without reloading the iframe.
    // NOTE: We do NOT set window.cordova because Ankama's code paths check it specially.
    // Instead we inject IonicDeeplink only, which lets the game register its deep-link handler.
    // The Electron code path (window.process.type) sets h=deepLink=dofustouch://authorized
    // and uses function c() to open the auth URL. Our navigation intercept in game-window.ts
    // catches the dofustouch:// redirect and fires OAUTH_CALLBACK.
    serveGameServer.get('/game/index.html', (req, res, next) => {
      const indexPath = join(GAME_PATH, 'index.html')
      if (originalFs.existsSync(indexPath)) {
        let content = originalFs.readFileSync(indexPath, 'utf8')
        content = content.replace(
          '<head>',
          '<head><script>window.IonicDeeplink = { onDeepLink: function(cb) { window.lindoDeeplinkCallback = cb; } };</script>'
        )
        res.send(content)
      } else {
        next()
    ...
    // Game files (script.js, styles-native.css, lindo patches, etc.)
    serveGameServer.use('/game', express.static(GAME_PATH, { index: false }))
    ...
    if (!rootStore.appStore._language) {
      ...
      logger.debug('Detected locale: ' + userLang)
  ...
  focusGameWindow(index: number) {
    logger.info(`Application.focusGameWindow called for index: ${index}, total windows: ${this._gWindows.length}`)
    if (this._gWindows[index]) {
      if (this._gWindows[index].isMinimized()) this._gWindows[index].restore()
      this._gWindows[index].focus()
    } else {
      logger.warn(`No game window found at index ${index}`)
    }
  }
  ...
  async createGameWindow(team?: GameTeam, teamWindow?: GameTeamWindow, characterId?: string) {
    ...
    const gWindow = await GameWindow.init({
      ...
      teamWindow,
      characterId,
      onFocusWindow: (targetIndex: number) => this.focusGameWindow(targetIndex)
    })
    gWindow.on('close', () => {
  ...
  private _setupIPCHandlers() {
    ...
    ipcMain.on(IPCEvents.OPEN_GAME_WINDOW, (event, characterId) => {
      logger.debug('Application -> OPEN_GAME_WINDOW ' + characterId)
      this.createGameWindow(undefined, undefined, characterId)
    })
    ...
    ipcMain.handle(IPCEvents.FETCH_GAME_CONTEXT, (_event, _context: string) => {
      // NOTE: The original lindo-app.com stats endpoint is no longer maintained.
      // This handler returns true (success) as a no-op to keep the renderer happy.
      logger.debug('Application -> FETCH_GAME_CONTEXT (no-op)')
    ...
    ipcMain.on(IPCEvents.RESET_GAME_DATA, () => {
      ...
      if (app.isPackaged) {
        ...
        // In dev mode, app.relaunch() has no effect (the yarn dev watcher manages Electron).
        // Just quit so the watcher re-spawns Electron automatically on next file save.
        app.quit()
      }
    })
