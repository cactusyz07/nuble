...
export const GameStoreModel = types
  ...
  .actions((self) => ({
    gamesFromTeamWindow(context: MultiAccountContext) {
      const teamWindow = self.rootStore.optionStore.gameMultiAccount
        .selectTeamById(context.teamId)
        ?.windows.find((w) => w.id === context.teamWindowId)
      if (context.characterId) {
        const character = teamWindow?.characters.find((c) => c.id === context.characterId)
        if (character) {
          self.addGame(character)
        }
      } else {
        teamWindow?.characters.forEach((character) => {
          self.addGame(character)
        })
      }
    },
