    ...
    <Observer>
      {() => (
        <Box sx={{ flexGrow: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ p: 1, borderBottom: 1, borderColor: 'divider', width: '100%' }}>
            <Tabs value={value} onChange={handleChange} aria-label='shortcuts-tabs' variant='fullWidth' sx={{ '& .MuiTab-root': { minWidth: 'auto', px: 1, py: 1, fontSize: '13px', fontWeight: 600, textTransform: 'none' } }}>
          ...
          <Box sx={{ p: 1, flexGrow: 1, overflowY: 'auto', overflowX: 'hidden', maxHeight: 'calc(100vh - 120px)' }}>
            <Box>
              ...
              <TabPanel value={value} index={6}>
                <Grid component='form' noValidate autoComplete='off' container spacing={1} margin={0}>
                    ...
                    <Stack spacing={2}>
                      {Array.from({ length: 5 }, (_, k) => k).map((i) => (
                        <ShortcutInput
                          id={'window-' + i}
                          key={'window-' + i}
                          label={LL.option.shortcuts.application.window({ x: i + 1 })}
                          value={hotkeyStore.window.windows[i]}
                          onChange={(shortcut) => hotkeyStore.window.setWindow(shortcut, i)}
                          restrictKeyCode
                        />
                      ))}
                    </Stack>
                  ...
                  <Grid item xs={6}>
                    <Stack spacing={2}>
                      {Array.from({ length: 5 }, (_, k) => k + 5).map((i) => (
                        <ShortcutInput
                          id={'window-' + i}
                          key={'window-' + i}
                          label={LL.option.shortcuts.application.window({ x: i + 1 })}
