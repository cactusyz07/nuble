...
const subscribeToOAuthCallback = (callback: (url: string) => void): (() => void) => {
  const listener = (_: IpcRendererEvent, url: string) => {
    callback(url)
  }
  ipcRenderer.on(IPCEvents.OAUTH_CALLBACK, listener)

  return () => {
    ipcRenderer.removeListener(IPCEvents.OAUTH_CALLBACK, listener)
  }
}
...
const openGameWindow = (characterId: string) => {
  ipcRenderer.send(IPCEvents.OPEN_GAME_WINDOW, characterId)
}
